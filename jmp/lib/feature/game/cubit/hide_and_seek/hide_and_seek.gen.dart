...
abstract class $HideAndSeekStateCopyWith<$Res> {
  ...
  @useResult
  $Res call({HideAndSeekStatus status});
}
...
class _$HideAndSeekStateImpl implements _HideAndSeekState {
  const _$HideAndSeekStateImpl({this.status = HideAndSeekStatus.initializing});
  ...
  @override
  String toString() {
    return 'HideAndSeekState(status: $status)';
  }
...
abstract class _HideAndSeekState implements HideAndSeekState {
  const factory _HideAndSeekState({final HideAndSeekStatus status}) =
      _$HideAndSeekStateImpl;
